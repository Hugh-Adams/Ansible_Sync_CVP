...
description:
  - CloudVision Portal Device compares the list of Devices in
...
def build_existing_devices_list(module):
    ...
    for cvp_device in facts_device:
        ...
        if is_in_filter(
            ...
            MODULE_LOGGER.debug(
                " * build_existing_devices_list - device matched is: %s", 
                str(cvp_device["hostname"]))
...
def devices_new(module):
    """
    Method to manage device provisioning.
    ...
    >>> devices_new(module=module)
    {
        ...
        "provisionned": [
            {
                "veos-01": "provisioning-tasks-['118']"
            }
        ],
            ...
            if "errorMessage" in str(device_action):
                ...
                if "taskIds" in str(device_action):
                    ...
                    result_update.append(
                        {
                            device_update["name"]: "provisioning-tasks-%s"
...
def devices_action(module):
    ...
    Action workorders and output tasks
